	...
	OrgFreedesktopDBus orgFreedesktopDBus)
{
	...
	private static StatusNotifierItemProperties CreateProperties(OrgKdeStatusNotifierItem.Properties item)
	{
		return new StatusNotifierItemProperties()
		{
			...
			IconPixmap = item.IconPixmap?.Select(i => GdkPixbufFactory.From(ImageHelper.ConvertArgbToRgba(i.Item3, i.Item1, i.Item2), 32, i.Item1, i.Item2)).ToArray(),
			OverlayIconName = item.OverlayIconName,
			OverlayIconPixmap = item.OverlayIconPixmap?.Select(i => GdkPixbufFactory.From(ImageHelper.ConvertArgbToRgba(i.Item3, i.Item1, i.Item2), 32, i.Item1, i.Item2)).ToArray(),
			AttentionIconName = item.AttentionIconName,
			AttentionIconPixmap = item.AttentionIconPixmap?.Select(i => GdkPixbufFactory.From(ImageHelper.ConvertArgbToRgba(i.Item3, i.Item1, i.Item2), 32, i.Item1, i.Item2)).ToArray(),
